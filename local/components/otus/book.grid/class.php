...
class BookGrid extends \CBitrixComponent implements Controllerable, Errorable
{
    ...
    /**
     * @return array
     * Возврат информации о действиях, доступных для совершения над строкой
     */
    private function getElementActions(array $fields): array
    {
        return [
            [
                'onclick' => "window.open('http://otus-05.localhost/bitrix/admin/perfmon_row_edit.php?lang=ru&table_name=aholin_book&pk%5BID%5D={$fields['ID']}')", // метод обработчик в js
    ...
    /**
     * @return array
     * Вовзрат массива с описанием колонок таблицы main.ui.grid
     */
    ...
    /**
     * @return array
     * Возврат кнопок, которые будут на странице
     */
    ...
    /**
     * @return void
     * Вызов шаблона компонента (Стандартный метод компонентов Битрикс)
     */
     ...
     * @throws \Bitrix\Main\SystemException
     * @return void
     * Подготовка данных для таблицы main.ui.grid
     */
    private function prepareGridData(): void
    {
        ...
        // [['ID' => 1], ['ID' => 2]] => [1, 2] - работа array_column
     ...
     * @param Result $books
     * @return array
     * Преобразование сырых данных к формату main.ui.grid
     */
    ...
    /**
     * @return array
     * Возврат полей для фильтрации по ним
     */
